...
const OmniOrchestrator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8004/api/v1/orchestrator/process', {
        method: 'POST',
        headers: {
      ...
      const assistantMessage = {
        role: 'assistant',
        content: data.final_answer || '抱歉，未收到回复。',
  ...
  const handleAction = (action) => {
    if (action.type === 'navigation' && action.url) {
      navigate(action.url);
      setIsOpen(false);
    } else if (action.type === 'continue_qa' || action.type === 'view_detail') {
      // 跳转到知识馆长页面
      navigate('/knowledge-curator');
      setIsOpen(false);
      ...
      <div className="flex flex-col space-y-3 w-full">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {msg.suggestedActions.map((action, idx) => (
              <button
                ...
                className="px-3 py-1.5 text-xs bg-vermilion/10 text-vermilion rounded-full hover:bg-vermilion/20 transition-colors flex items-center gap-1"
              >
                {action.type === 'navigation' && <Sparkles size={12} />}
                {action.label}
              </button>
            ))}
          </div>
        ...
        {msg.imageUrl && (
          <div className="mt-3">
            ...
            />
            <p className="text-xs text-gray-500 mt-1 text-center">点击图片可查看大图</p>
          </div>
        )}
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <div className="text-sm text-gray-500">
                ...
                <p className="mt-4 text-xs italic">试试发送："我想学苏绣" 或 "去视觉导师"</p>
              </div>
            </div>
          ) : (
              ...
              {isLoading && (
                <div className="flex justify-start">
                  ...
                  <div className="bg-white border border-gray-100 rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm flex items-center space-x-2">
                    ...
                    <span className="text-xs text-gray-500 ml-2">正在处理中...</span>
                  </div>
                </div>
              )}
