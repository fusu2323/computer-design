...
```
  ...
  <verify>
    <automated>grep -c "latitude\|longitude" backend/app/db/seed_coordinates.py && python -c "import ast; ast.parse(open('backend/app/db/seed_coordinates.py').read()); print('SYNTAX_OK')"</automated>
  </verify>
  <done>seed_coordinates.py exists with HERITAGE_COORDINATES array with lat/lng for >= 10 ICH sites. seed_coordinates() returns (count, names). verify_coordinates() returns (count, names), raises RuntimeError if count < 5. Main block aborts with SystemExit(1) on verification failure.</done>
</task>
...
<task type="auto">
  <name>Task 4: Create pytest test suite for map API</name>
  <files>backend/tests/test_map_api.py</files>
  <action>
Create file `backend/tests/test_map_api.py` with pytest tests for the map API endpoints.

Note: This is primarily for backend API verification. The map page itself requires manual verification via WeChat DevTools.
...
```
  ...
  <verify>
    <automated>grep -c "def test_" backend/tests/test_map_api.py</automated>
  </verify>
  <done>backend/tests/test_map_api.py exists with pytest tests for /map/markers and /map/nearby endpoints</done>
</task>
...
- [ ] `backend/tests/test_map_api.py` exists with pytest tests for both endpoints
