...
# 导航意图关键词映射
ROUTE_KEYWORDS = {
    "/vision-mentor": ["视觉导师", "手势", "身段", "姿态", "学习", "练习", "动作"],
    "/knowledge-curator": ["知识馆长", "知识", "问答", "查询", "了解", "历史"],
    "/creative-workshop": ["创意工坊", "创作", "生成图片", "画", "设计", "海报"],
    "/craft-library": ["工艺库", "工艺品", "技艺"],
    "/my-practice": ["我的练习", "学习进度", "进度"],
}
...
JSON 格式要求如下：
{
    ...
    "reply": "如果是 unknown，请给出一个友好的回退提示（如：抱歉，我没能理解您的意图。您可以问我关于非遗的知识，让我帮您制定学习计划，或者为您创作非遗相关的图片）；如果是其他意图，请留空字符串。"
        ...
        try:
            ...
            # 检测导航意图
            result["suggested_actions"] = self._detect_navigation(query)
    ...
    def _get_fallback_result(self, query: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "reply": reply,
            "suggested_actions": self._detect_navigation(query)
        }

    def _detect_navigation(self, query: str) -> List[Dict[str, Any]]:
        """检测用户是否想要导航到特定页面"""
        suggested_actions = []
        lower_query = query.lower()
        ...
        for route, keywords in ROUTE_KEYWORDS.items():
            if any(kw in lower_query for kw in keywords):
                label_map = {
                    "/vision-mentor": "去视觉导师",
                    "/knowledge-curator": "去知识馆长",
                    "/creative-workshop": "去创意工坊",
                    "/craft-library": "去工艺库",
                    "/my-practice": "查看学习进度"
                }
                suggested_actions.append({
                    "type": "navigation",
                    "label": label_map.get(route, f"去{route}"),
                    "url": route
                })
