...
class OrchestratorService:
    ...
    async def process_request(self, query: str, session_id: str = None, history: List[Dict] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "image_url": self._extract_image_url(results)
        }

    def _extract_image_url(self, results: Dict[str, Any]) -> str:
        """从结果中提取图片URL"""
        print(f"[DEBUG] _extract_image_url called with: {results}")
        for task_id, result in results.items():
            print(f"[DEBUG] Checking task_id={task_id}, result type={type(result)}, starts_with_http={isinstance(result, str) and result.startswith('http')}")
            if isinstance(result, str) and result.startswith("http"):
                # 图片URL通常是s3或其他CDN的链接，包含image或output等关键词
                if "s3" in result.lower() or "image" in result.lower() or "output" in result.lower() or ".png" in result.lower() or ".jpg" in result.lower():
                    print(f"[DEBUG] Found image URL: {result[:50]}")
                    return result
