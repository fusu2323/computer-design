...
- **Global scope**: `D:/computer-design/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/computer-design/.claude/notes/`)
...
2. Glob `D:/computer-design/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/computer-design/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/computer-design/.claude/notes/` — works in any directory
